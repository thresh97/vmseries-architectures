...
| **Azure** | Gateway Load Balancer (GWLB) | Yes — VXLAN encapsulation | Primarily N-S Inbound; chained to a public-facing LB |
| **Azure** | External Load Balancer (ELB) | Yes (Inbound) / No (Outbound) | Inbound: public-facing ingress, client IP preserved to backend. Outbound: SNAT via outbound NAT rules for egress |
...
| **GCP** | Internal Passthrough Network Load Balancer | Yes — GENEVE encapsulation; instance-based flow tracking ensures symmetry across firewall cluster | E/W inspection; traffic steered via policy-based routing |
